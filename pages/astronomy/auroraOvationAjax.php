<?php

	############################################################################
	#
	#	Norrskenskarta - AJAX-data
	#	Namespace:		aurora
	#	Meteotemplate-sida (custom, not from meteotemplate.com)
	#
	#	v1.0 - Sep 23, 2026
	#		- filtrerar NOAA:s globala OVATION-grid (getAuroraOvationGrid())
	#		  till ett fönster runt Skandinavien/Nordatlanten, hittar
	#		  samtidigt närmaste gridpunkt till stationens koordinater
	#
	############################################################################

	include("../../config.php");
	include($baseURL."css/design.php");
	include($baseURL."scripts/functions.php");

	$grid = getAuroraOvationGrid();

	$points = array();
	$stationValue = null;
	$stationDist = null;

	if(isset($grid['coordinates']) && is_array($grid['coordinates'])){
		foreach($grid['coordinates'] as $c){
			$lon = $c[0];
			$lat = $c[1];
			$val = $c[2];

			// OVATION anger longitud 0-359, Leaflet vill ha -180..180
			if($lon>180){
				$lon = $lon - 360;
			}

			// avgränsar till ett fönster runt Skandinavien/Nordatlanten,
			// annars blir svaret onödigt stort (hela jorden är ~65000 punkter)
			if($lat<35 || $lon<-40 || $lon>70){
				continue;
			}

			// hittar närmaste gridpunkt till stationen (1-graders upplösning,
			// enkelt avstånd i grader räcker för "närmaste")
			$dist = abs($lat-$stationLat) + abs($lon-$stationLon);
			if($stationDist===null || $dist<$stationDist){
				$stationDist = $dist;
				$stationValue = $val;
			}

			if($val>0){
				$points[] = array($lat,$lon,$val);
			}
		}
	}

	$result = array(
		"points" => $points,
		"stationValue" => $stationValue,
		"observationTime" => isset($grid['Observation Time']) ? $grid['Observation Time'] : null,
		"forecastTime" => isset($grid['Forecast Time']) ? $grid['Forecast Time'] : null
	);

	header('Content-Type: application/json');
	print json_encode($result);

?>
