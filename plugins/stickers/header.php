<?php
	...
	$time = time();
	$suninfo = date_sun_info($time, $stationLat,$stationLon);
    $sunRise = date('H:i', $suninfo['sunrise']);
	$sunSet = date('H:i', $suninfo['sunset']);
    $sunRiseIB = $suninfo['sunrise'];
	$sunSetIB = $suninfo['sunset'];
	

	if($interactiveBanner){	
		//if( (time()>$sunRiseIB) && (time()<$sunSetIB) ){
		if(time()>$sunRiseIB && time()<$sunSetIB){	
			$dayTime = "d";
			$moonView = false;
		}
		...
		$intervals = 118;
		$moonIcon = round($img_phase*$intervals);
		...
		if(file_exists($baseURL."cache/interactiveBanner.txt")){
			...
			file_put_contents($baseURL."cache/metarConditions.txt",$metarDataString);
			$metarDataString = cleanHTML($metarDataString);
			file_put_contents($baseURL."cache/metarConditions.txt",$metarDataString,FILE_APPEND);
    ...
    // echo "Daytime: $dayTime - Conditions: $condition - Metar: $metarDataString" ;
		...
		if($condition=="few" || $condition=="partly"){
			if($dayTime=="d"){
				// $bgImage = "day_scattered".rand(1,4);
				$bgImage = "scattered".rand(1,4);
			}
			if($dayTime=="n"){
				$bgImage = "night_cloudy".rand(1,3);
				$moonView = true;
			}
		}
		if($condition=="mostly"){
			if($dayTime=="d"){
				// $bgImage = "day_cloudy".rand(1,4);
		...
		if($condition=="overcast" || $condition=="drizzle" || $condition=="lightrain" || $condition=="rain"){
			if($dayTime=="d"){
				// $bgImage = "day_overcast".rand(1,5);
				$bgImage = "overcast".rand(1,5);
			}
			if($dayTime=="n"){
				$bgImage = "night_overcast".rand(1,2);
				$moonView = false;
			}
		}
		if($condition=="fog"){
		...
		if($condition=="snow"){
			if($dayTime=="d"){
				// $bgImage = "day_snow".rand(1,2);
				$bgImage = "snow".rand(1,2);
			}
			if($dayTime=="n"){
				$bgImage = "night_overcast";
				$moonView = false;
			}
		}
		if($condition=="thunderstorm"){
			if($dayTime=="d"){
				// $bgImage = "day_thunder".rand(1,4);
				$bgImage = "thunder".rand(1,4);
			}
			if($dayTime=="n"){
				$bgImage = "night_thunder".rand(1,3);
				$moonView = false;
			}
		}
		if($condition=="NA"){
...
?>
		...
		?>
				...
				<script src="<?php echo $pageURL.$path?>scripts/cookieconsent.js"></script>
				<script>
	...
	<div id="header" style="position:relative">
		...
		?>
					...
					<div style="position:absolute;bottom:10px;right:10px;z-index:10;background:#<?php echo $color_schemes[$design2]['700']?>;padding-left:5px;padding-right:5px;border:1px solid #<?php echo $color_schemes[$design2]['500']?>;border-radius:10px;" id="warningsDiv">
						<?php
						    if(( isset($stationWarningsArray['highTWarning']))==true){
						?>
						...
						?>
						<?php
						    if(( isset($stationWarningsArray['lowTWarning']))==true){
						?>
						...
						?>
						<?php
						    if(( isset($stationWarningsArray['highWWarning']))==true){
						?>
						...
						?>
						<?php
							if(( isset($stationWarningsArray['highRWarning']))==true){
						?>
						...
						<?php
							if($solarSensor){
								if(( isset($stationWarningsArray['highSWarning']))==true){
						?>
