...
-------------------------
*/
$buffer_time_metar = 0.5;                // hours for buffering METAR info
$buffer_time_taf   = 0.5;                // hours for buffering TAF info
$buffer_night_yes  = 1;                // if METAR shall be buffered during night = 1
$path_to_buffer    = $baseURL."/pages/metar/buffer/";        // relative path to the buffer directory

$path_to_grafics   = "weatherpics/";   // relative path to the weatherpic directory

$type_of_db_acces  = "text";           // shall be either "text" or "mysql"

$path_to_icao_db   = $baseURL."/pages/metar/icao_db/";       // relative path to the icao_db directory
...
function getMetarFromWWW($ICAO) {
         ...
         if ($metardata = getMetarFromBuffer($ICAO)) {
            ...
            if ($metarArray=@file(getN("METAR")."$ICAO.TXT")) {
               $datei=fopen($path_to_buffer . "" . $ICAO . ".TXT","w");
...
function getStationInfo($ICAO) {
         ...
         if (preg_match("/^([A-Z]{4})$/", $ICAO)) {
            $firstletter = strtolower(substr($ICAO,0,1));
            include ($path_to_icao_db."icao_$firstletter.inc.php");
			$returner["station"] = $station[$ICAO] ?? null;
            $returner["country"] = $country[$ICAO] ?? null;
            $returner["gmttime"] = $gmttime[$ICAO] ?? null;
            if ($returner["gmttime"] == "0") $returner["gmttime"] = "+/-0";
            $we = "E";
            $ns = "N";
            if (( isset($longitude[$ICAO])) < 0) $we = "W" ;
            if (( isset($latitude[$ICAO])) < 0) $ns = "S" ;
			$returner["longitude"] = utf8_encode(HMSmaker(abs($longitude[$ICAO] ?? 0)) . $we);
            $returner["latitude"] = utf8_encode(HMSmaker(abs($latitude[$ICAO] ?? 0)) . $ns);
            $returner["altitude"] = $altitude[$ICAO] ?? null;
            $returner["altimeter"] = round($altitude[$ICAO] ?? null * 0.305) ;
         }
...
function getWeatherGrafic($tafcode,$icao) {
         ...
         if (preg_match("/(mist)|(fog)|(smoke)|(haze)|(dust)|(sand)|(ash)/", $weatherCond)) $pic = "fog";
...
function getClouds($tafcode,$number) {
         ...
         for ($a=0;$a<count($tafItemArray);$a++) {
             ...
             if (($first3=="NSC") || ($first3=="CLR") || ($first3=="SKC") || ($first3=="FEW") || ($first3=="SCT") || ($first3=="BKN") || ($first3=="OVC") || ($first3=="CAV")) {
                if ($number==$aktualCloudBase++) {
                   ...
                   if (($first3 == "SKC") || ($first3 == "CLR")) {
                      ...
                      $returner["feet"] = 100*intval(substr($tafItemArray[$a],3,3));
